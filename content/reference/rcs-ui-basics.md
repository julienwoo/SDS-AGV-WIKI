...
## 1. AGV Work State in RCS (Vehicle Status)

Vehicle List page, **Vehicle Status** field:

| Vehicle Status | Meaning |
|---|---|
| Invalid | No valid status reading from this AGV |
| Standby | Idle, not executing a task |
| Auto Mode | In Automatic mode |
| Manual Mode | In Manual mode |
| Abnormal | Active error/fault |
| Shutdown | Powered off |
| Offline | Not connected to RCS |

Different from the AGV's own status vocabulary in [AGV Status Meanings](agv-status.md) — the two systems don't map one-to-one.

## 2. AGV Operating Mode
...
## 3. Vehicle and StorageBin Lock / Unlock
...
## 5. StorageBin Occupancy and Usage Status
...
- "Lock" mark (bottom-right) — currently reserved (see section 3)
