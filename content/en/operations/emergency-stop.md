...
## ⚠️ Safety rule for every fault recovery — read this once

Before doing any manual, hands-on step in the Troubleshooting section — clearing an obstacle, checking a sensor, adjusting a pallet, reaching anywhere near the AGV — **press E-Stop first, even if the AGV already looks stopped.** Confirm the area is genuinely safe before you touch anything. Only release E-Stop once you've stepped clear and confirmed it's safe to resume, following the release procedure below. This one rule applies to every recovery step across the whole Troubleshooting section, so it isn't repeated on each individual page — this page is the one place it's spelled out.
