...
2. Have a supervisor or trained team member open **RoboTune** (browser interface, same Wi-Fi network as the AGV — see the AGV's network setup in [AGV Power On / Off](power-on-off.md)) and use its relocalization function, or follow your site's own documented commissioning/relocalization SOP if one exists.
